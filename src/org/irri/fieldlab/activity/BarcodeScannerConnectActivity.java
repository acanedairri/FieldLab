...
public class BarcodeScannerConnectActivity extends Activity {
	...
	private String current_Device_Status;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		current_Device_Status=extras.getString("CURRENT_DEVICE_STATUS");
		...
		btnFindReader.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				if (btStackAtStartup == false) {
					BarcodeScannerConnectActivity.this.startActivityForResult(new Intent(BluetoothAdapter.ACTION_REQUEST_ENABLE),
							BarcodeScannerConnectActivity.ACTION_REQUEST_ENABLE_BT_RESULT);
				} else {
					// Start the Select Device activity from the BaracodaManager by specifying the right intent
					BarcodeScannerConnectActivity.this.startActivityForResult(new Intent("com.baracoda.android.baracodamanager.SelectDeviceActivity"), BarcodeScannerConnectActivity.ACTION_REQUEST_SEARCH_DEVICES_RESULT);
				}
		...
		btnConnectReader.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				try {
					...
					if(BarcodeScannerConnectActivity.this.baracodaService.getConnectionState() != ConnectionState.DISCONNECTED  ) {
						...
						BarcodeScannerConnectActivity.this.baracodaService.disconnect();
						currentDeviceView.setText("");
						btnConnectReader.setEnabled(false);
					...
					// If we are disconnected then connect the current device
						...
						BarcodeScannerConnectActivity.this.baracodaService.disconnect();
						BarcodeScannerConnectActivity.this.baracodaService.connect(BarcodeScannerConnectActivity.this.getCurrentDevice().getAddress());
					}
		...
		if(extras.getInt("CONNECTION_STATE")==2){
			currentDeviceView.setText(extras.getString("CURRENT_DEVICE"));
	...
	private final ServiceConnection baracodaServiceConnection = new ServiceConnection () {
		public void onServiceConnected(ComponentName className, IBinder service) {
			BarcodeScannerConnectActivity.this.baracodaService = IBaracodaReaderService.Stub.asInterface(service);

			try {
				BarcodeScannerConnectActivity.this.baracodaService.registerCallback(BarcodeScannerConnectActivity.this.baracodaServiceCallback);

				// We are not using application ack in this example, let's save its state
				applicationAckAtStartup = BarcodeScannerConnectActivity.this.baracodaService.getApplicationAck();
				// Disable application ack
				BarcodeScannerConnectActivity.this.baracodaService.setApplicationAck(false);

				// We are not using raw mode in this example, let's save its state
				rawModeAtStartup = BarcodeScannerConnectActivity.this.baracodaService.getRawMode();
				// Disable raw mode
				BarcodeScannerConnectActivity.this.baracodaService.setRawMode(false);
			}
			catch (RemoteException e) {
				// nothing to do here
				Toast.makeText(BarcodeScannerConnectActivity.this, "Error" + e.getMessage(), Toast.LENGTH_LONG).show();
			}
		}

		public void onServiceDisconnected(ComponentName className) {
			BarcodeScannerConnectActivity.this.baracodaService = null;
		}
	};
	...
	private final IBaracodaReaderServiceCallback.Stub baracodaServiceCallback = new IBaracodaReaderServiceCallback.Stub() {
		...
		public void onAutoConnectChanged() throws RemoteException {
			...
			BarcodeScannerConnectActivity.this.messageHandler.obtainMessage(BarcodeScannerConnectActivity.MESSAGE_ON_AUTOCONNECT_STATE_CHANGED,
					0, 0).sendToTarget();
		}
		...
		public void onConnectionStateChanged(int connectionState) throws RemoteException {
			...
			BarcodeScannerConnectActivity.this.messageHandler.obtainMessage(BarcodeScannerConnectActivity.MESSAGE_ON_CONNECTION_STATE_CHANGED,
		...
		public void onDataRead(int dataType, String dataText) throws RemoteException {
			...
			BarcodeScannerConnectActivity.this.messageHandler.obtainMessage(
					BarcodeScannerConnectActivity.MESSAGE_ON_DATA_READ,
					dataType,
					-1,
		...
		public void onReaderChanged() throws RemoteException {
			// We need to switch to our handler (to avoid threading issues with GUI)
			BarcodeScannerConnectActivity.this.messageHandler.obtainMessage(BarcodeScannerConnectActivity.MESSAGE_ON_READER_CHANGED).sendToTarget();
		}
	...
	private final Handler messageHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			switch (msg.what) {
			case BarcodeScannerConnectActivity.MESSAGE_ON_READER_CHANGED:
				// Current reader has changed, update the text field


				try {
					currentDeviceView.setText(String.format("%s, %s", BarcodeScannerConnectActivity.this.baracodaService.getName(),
							BarcodeScannerConnectActivity.this.baracodaService.getAddress()));
			...
			case BarcodeScannerConnectActivity.MESSAGE_ON_CONNECTION_STATE_CHANGED:
				...
				if(msg.arg1 == ConnectionState.CONNECTED) {
					...
					try {
						...
						if(BarcodeScannerConnectActivity.this.baracodaService.getAutoConnect() == true) {
							btnConnectReader.setEnabled(true);
						...
						bundle.putInt("CONNECTION_STATE", 2);
						bundle.putString("CURRENT_DEVICE", currentDeviceView.getText().toString());
...
//						Intent baracodaServiceIntent = new Intent (IBaracodaReaderService.class.getName());
			...
			try {
				int x = BarcodeScannerConnectActivity.this.baracodaService.getConnectionState();
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		Log.d(BarcodeScannerConnectActivity.TAG, "SettingsActivity.onActivityResult");
		...
		case BarcodeScannerConnectActivity.ACTION_REQUEST_SEARCH_DEVICES_RESULT:
			if (resultCode == Activity.RESULT_OK) {
				...
				try {
					// Get the reader info
					BluetoothDevice device = data.getParcelableExtra(BarcodeScannerConnectActivity.EXTRA_DEVICE);
					...
					Toast.makeText(BarcodeScannerConnectActivity.this, "Error ACTION_REQUEST_SEARCH_DEVICES_RESULT:" + e.getMessage(), Toast.LENGTH_LONG).show();
				}
			}
			break;
		}
